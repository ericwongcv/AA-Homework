...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    ...
    FROM
      routes a
    JOIN
      routes b ON (b.company = a.company AND b.num = a.num)
    WHERE
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      end_route_stops.name, end_routes.company, end_routes.num
    FROM
      routes AS start_routes
    JOIN
    ...
    JOIN
      stops AS end_route_stops ON end_route_stops.id = end_routes.stop_id
    JOIN
      stops AS origin_stops ON origin_stops.id = start_routes.stop_id
    WHERE
      origin_stops.name = 'Craiglockhart';
